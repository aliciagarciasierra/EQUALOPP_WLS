...
To assess whether our samples are adequately powered to detect the within-family PGI contribution to inequality of opportunity, we conducted a simulation study targeting delta_within = (emptyind − condind) / totalvar, the share of total outcome variance attributable to PGIs through within-family variation (reduction from model M0 to M1). Simulation parameters were calibrated from the observed data: for the WLS Brothers subsample (N = 473 families) the observed ICC is 0.377 and delta_within = 0.038; for Sisters (N = 612 families), ICC = 0.373 and delta_within = 0.033. In each replication, we generated synthetic sibling datasets reproducing this variance structure by partitioning each PGI into a between-family and a within-family component, calibrating the PGI coefficients so that the within-family genetic contribution equals a pre-specified true delta_within, and setting residual variances to match the observed ICC exactly. We then fit models M0 and M1 via restricted maximum likelihood and extracted the estimated delta_within. The null distribution was built from 500 replications at delta_within = 0 using each subsample's observed N; the 95th percentile served as the critical value for a one-sided test at alpha = 0.05. Power at each point on an effect-size grid was estimated as the proportion of 500 replications in which the estimate exceeded this critical value. The same procedure was applied to the MoBa gender-stratified subsamples (Brothers: N = 1,605 families, ICC = 0.433, delta_within = 0.001; Sisters: N = 3,458 families, ICC = 0.368, delta_within = 0.010), using heritability parameters calibrated from Howe et al. (2022). Figure X reports the resulting power curves for all subsamples, with the observed effect size and the minimum detectable effect at 80% power indicated for each.
