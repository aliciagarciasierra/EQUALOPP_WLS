...
# Compute SC, Liberal IOP, Radical IOP
...
ci_summary <- mutate(ci_summary, N = as.character(N))
...
ggplot(ci_summary, aes(x = Outcome, y = Estimate, fill = Index)) +
  ...
  # Save the plot
ggsave(paste0("plots/",natural_talents,"_age_filter.png"), width = 13, height = 6, dpi = 300)
...
outcomes <- unique(ci_summary$Outcome)  # This will grab the unique outcomes from your data

# Filter data for the current outcome
data_subset <- subset(ci_summary, Outcome == outcome)
