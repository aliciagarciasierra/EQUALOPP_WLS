...
for (outcome in outcomes) {
  ...
  p <- ggplot(data_subset, aes(x = Outcome, y = Estimate, fill = Index)) +
    ...
    theme(
      ...
      panel.grid.major = element_blank(),  # Remove major grid lines
      panel.grid.minor = element_blank(),  # Remove minor grid lines
      panel.background = element_rect(fill = "#E0F7FA", color = NA),  # Set panel background to cream
      plot.background = element_rect(fill = "#E0F7FA", color = NA),  # Set plot background to cream
      plot.margin = margin(10, 10, 10, 10),  # Add space around the plot
      legend.background = element_rect(fill = "#E0F7FA")
      )
  
  # Save the plot for the current outcome
  ggsave(paste0("plots/results_plot_pgis_", outcome, ".pdf"), plot = p, width = 13, height = 6, dpi = 300)
}
