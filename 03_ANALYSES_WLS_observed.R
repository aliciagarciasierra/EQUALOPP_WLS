...
saveWorkbook(wb, paste0("full_results_",OBSERVED_COG,".xlsx"), overwrite = TRUE)
...
# Read the data 
data_graph <- read_excel(paste0("full_results_",OBSERVED_COG,".xlsx"), sheet = "For plotting")
...
#
# Save the plot
ggsave(paste0("plots/results_plot_observed.pdf"), width = 13, height = 6, dpi = 300)
...
outcome_titles <- c(
  "education" = "Education with observed abilities",
  "occupation" = "Occupation with observed abilities",
  "income" = "Household Income with observed abilities",
  "wealth" = "Household Wealth with observed abilities",
  "health_pc" = "Health with observed abilities"
)
...
# Loop through each outcome and save a separate plot
for (outcome in outcomes) {
  ...
  p <- ggplot(data_subset, aes(x = Outcome, y = Estimate, fill = Index)) +
    ...
    theme(
      ...
      panel.grid.major = element_blank(),  # Remove major grid lines
      panel.grid.minor = element_blank(),  # Remove minor grid lines
      panel.background = element_rect(fill = "ivory", color = NA),  # Set panel background to cream
      plot.background = element_rect(fill = "ivory", color = NA),  # Set plot background to cream
      plot.margin = margin(10, 10, 10, 10),  # Add space around the plot
      legend.background = element_rect(fill = "ivory")
      )
  
  # Save the plot for the current outcome
  ggsave(paste0("plots/results_plot_observed_", outcome, ".pdf"), plot = p, width = 13, height = 6, dpi = 300)
}
